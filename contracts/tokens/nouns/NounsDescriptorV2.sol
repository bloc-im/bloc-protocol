...
import { NFTDescriptorV2 } from './NFTDescriptorV2.sol';
...
contract NounsDescriptorV2 is INounsDescriptorV2, Ownable {
    ...
    function getPartsForSeed(INounsSeeder.Seed memory seed) public view returns (ISVGRenderer.Part[] memory) {
        bytes memory body_ = art.bodies(seed.body);
        bytes memory accessory_ = art.accessories(seed.accessory);
        bytes memory head_ = art.heads(seed.head);
        bytes memory glasses_ = art.glasses(seed.glasses);
        bytes memory palette_ = art.palettes(0);

        ISVGRenderer.Part[] memory parts = new ISVGRenderer.Part[](4);

        parts[0] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[1] = ISVGRenderer.Part({ image: accessory_, palette: palette_ });
        parts[2] = ISVGRenderer.Part({ image: head_, palette: palette_ });
        parts[3] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        return parts;
    }

    function getPartForBytes(bytes memory _body) public view returns (ISVGRenderer.Part memory) {
        bytes memory palette_ = art.palettes(0);
        return ISVGRenderer.Part({ image: _body, palette: palette_ }); 
    }
  ...
  /**
     * @notice Get all Noun parts for the passed `seed`.
     */
    function getParts0() public view returns (ISVGRenderer.Part memory) {
        bytes memory body_ = art.bodies(0);
        // bytes memory accessory_ = art.accessories(0);
        // bytes memory head_ = art.heads(0);
        bytes memory glasses_ = art.glasses(0);
        bytes memory palette_ = art.palettes(0);

        ISVGRenderer.Part[] memory parts = new ISVGRenderer.Part[](16);

        parts[0] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[1] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[2] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[3] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[4] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[5] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[6] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[7] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[8] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[9] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[10] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[11] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[12] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[13] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[14] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[15] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
  ...
  /**
     * @notice Get all Noun parts for the passed `seed`.
     */
    function getParts1() public view returns (ISVGRenderer.Part memory) {
        bytes memory body_ = art.bodies(0);
        bytes memory accessory_ = art.accessories(0);
        // bytes memory head_ = art.heads(0);
        // bytes memory glasses_ = art.glasses(0);
        ...
        parts[0] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[1] = ISVGRenderer.Part({ image: accessory_, palette: palette_ });
        parts[2] = ISVGRenderer.Part({ image: accessory_, palette: palette_ });
        parts[3] = ISVGRenderer.Part({ image: accessory_, palette: palette_ });
        return parts[1];
    }
  ...
  /**
     * @notice Get all Noun parts for the passed `seed`.
     */
    function getParts2() public view returns (ISVGRenderer.Part memory) {
        bytes memory body_ = art.bodies(0);
        // bytes memory accessory_ = art.accessories(0);
        bytes memory head_ = art.heads(2);
        bytes memory glasses_ = art.glasses(0);
        bytes memory palette_ = art.palettes(0);

        ISVGRenderer.Part[] memory parts = new ISVGRenderer.Part[](4);

        parts[0] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        parts[1] = ISVGRenderer.Part({ image: head_, palette: palette_ });
        parts[2] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[3] = ISVGRenderer.Part({ image: head_, palette: palette_ });
        return parts[2];
    }
     ...
     * @notice Get all Noun parts for the passed `seed`.
     */
    function getParts3() public view returns (ISVGRenderer.Part memory) {
        bytes memory body_ = art.bodies(0);
        bytes memory accessory_ = art.accessories(0);
        bytes memory glasses_ = art.glasses(0);
        bytes memory palette_ = art.palettes(0);
        ...
        parts[0] = ISVGRenderer.Part({ image: body_, palette: palette_ });
        parts[1] = ISVGRenderer.Part({ image: accessory_, palette: palette_ });
        parts[2] = getPartForHead();
        parts[3] = ISVGRenderer.Part({ image: glasses_, palette: palette_ });
        return parts[2];
    }

    function getPartForBody() public view returns (ISVGRenderer.Part memory) {
        bytes memory body_ = art.bodies(0);
        bytes memory palette_ = art.palettes(0);
        return ISVGRenderer.Part({ image: body_, palette: palette_ });
    }
    
    function getPartForHead() public view returns (ISVGRenderer.Part memory) {
        bytes memory head_ = art.heads(0);
        bytes memory palette_ = art.palettes(0);
        return ISVGRenderer.Part({ image: head_, palette: palette_ });
    }
    
    function getPartForAccessory() public view returns (ISVGRenderer.Part memory) {
        bytes memory accessory_ = art.accessories(0);
        bytes memory palette_ = art.palettes(0);
        return ISVGRenderer.Part({ image: accessory_, palette: palette_ });
    }
    
    function getPartForGlasses() public view returns (ISVGRenderer.Part memory) {
        bytes memory glasses_ = art.glasses(0);
        bytes memory palette_ = art.palettes(0);
        return ISVGRenderer.Part({ image: glasses_, palette: palette_ });
    }

    function getPartsForBody(bytes memory _body, bytes memory _palette) public view returns (ISVGRenderer.Part[] memory) {
        ISVGRenderer.Part[] memory parts = new ISVGRenderer.Part[](1);
        parts[0] = ISVGRenderer.Part({ image: _body, palette: _palette });
        return parts;
    }

    function getZeroPalette() external view  returns (bytes memory) {
        return art.palettes(0);
    }

    function getPaletteIndex(uint8 index) external view  returns (bytes memory) {
        return art.palettes(index);
    }
    function getPaletteBytes(bytes memory part) external view  returns (bytes memory) {
        return _getPalette(part);
    }
    function getPaletteUint(bytes memory part) external view  returns (uint8) {
        return uint8(part[0]);
    }
